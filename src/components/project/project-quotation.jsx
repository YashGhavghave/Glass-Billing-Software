...
export function ProjectQuotation({ partyInfo, designs }) {
    const [isExporting, setIsExporting] = useState(false);
    const printAreaRef = useRef(null);
    useEffect(() => {
        document.title = `Quotation - ${partyInfo.billNo} - ${partyInfo.name}`;
    }, [partyInfo]);
    ...
    const handlePrint = async () => {
        ...
        try {
            ...
            const fileName = `Quotation-${partyInfo.billNo}.pdf`;
            pdf.save(fileName);
        }
        catch (error) {
            console.error("Error generating PDF:", error);
            // Fallback to browser print in case of an error
            window.print();
        }
        finally {
    ...
    return (<div className="bg-background min-h-screen">
      ...
      <main className="container mx-auto p-4 sm:p-6 md:p-8 font-sans">
        <div className="print-area">
            <Card ref={printAreaRef} className="w-full max-w-4xl mx-auto print:shadow-none print:border-none">
