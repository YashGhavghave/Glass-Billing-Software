...
const PADDING = 100;
export function Visualization({ geometry, isProcessing, parameters, panelOpenStates, panelOffsets, togglePanelOpenState, updatePanelOffsets }) {
    ...
    useEffect(() => {
        if (geometry) {
            if (geometry.frame.outer.width !== viewGeomRef.current.width ||
    ...
    const handlePanelClick = (e, index) => {
        if (isSliding)
            return;
        if (!isOperableUpDown && !isCasement && !isFoldable)
            return;
        e.stopPropagation();
        togglePanelOpenState(index);
    };
    const handlePanelMouseDown = (e, index) => {
        if (!isSliding)
            return;
        e.stopPropagation();
    ...
    const handleCanvasMouseMove = (e) => {
        ...
        if (!isDraggingCanvas || !svgRef.current || !geometry)
            return;
    ...
    return (<div className="relative w-full h-full bg-card rounded-lg border shadow-sm flex items-center justify-center overflow-hidden" onMouseDown={handleCanvasMouseDown} onMouseMove={handleCanvasMouseMove} onMouseUp={handleMouseUp} onMouseLeave={handleMouseUp} onWheel={handleWheel}>
      {(isProcessing || !geometry) && (<Skeleton className="absolute w-full h-full"/>)}
      {geometry && (<Button onClick={resetView} variant="ghost" size="icon" className="absolute top-2 right-2 z-10 h-9 w-9 bg-background/50 backdrop-blur-sm hover:bg-background/80" title="Reset View">
          <Maximize className="h-5 w-5"/>
        </Button>)}
      {geometry && (<svg ref={svgRef} className={cn("w-full h-full transition-opacity duration-300", isProcessing ? "opacity-50" : "opacity-100", isDraggingCanvas || draggedPanel ? 'cursor-grabbing' : 'cursor-grab')} viewBox={`${viewBox.x} ${viewBox.y} ${viewBox.width} ${viewBox.height}`} preserveAspectRatio="xMidYMid meet">
          <defs>
          ...
          <g transform={`translate(${PADDING}, ${PADDING})`}>
            ...
            {geometry.panels.map((panel, index) => {
                ...
                return (<g key={index} transform={transformString} onMouseDown={(e) => handlePanelMouseDown(e, index)} onClick={(e) => handlePanelClick(e, index)} style={{ transitionDuration: draggedPanel?.index === index ? '0ms' : '300ms' }} className={cn('transition-transform', ((isOperableUpDown || isCasement || isFoldable) && !isSliding) && 'cursor-pointer', isSliding && 'cursor-grab')}>
                  ...
                  <text x={panel.glassRect.x + panel.glassRect.width / 2} y={panel.glassRect.y + panel.glassRect.height / 2} textAnchor="middle" dominantBaseline="middle" fill="hsl(var(--foreground))" opacity="0.2" fontSize={40 / zoom} fontWeight="bold" className="pointer-events-none uppercase tracking-wider">
             ...
             {/* Tracks */}
            {geometry.tracks.map((track, index) => (<line key={index} {...track} stroke="hsl(var(--border))" strokeWidth={1 / zoom} strokeDasharray={`${4 / zoom} ${2 / zoom}`}/>))}
            ...
            <g className="text-muted-foreground" style={{ userSelect: "none" }}>
              ...
              <text x={geometry.frame.outer.width / 2} y={-60} textAnchor="middle" fill="hsl(var(--foreground))" fontSize={40 / zoom} className="font-semibold">
              ...
              <text x={geometry.frame.outer.width + 60} y={geometry.frame.outer.height / 2} textAnchor="middle" fill="hsl(var(--foreground))" transform={`rotate(-90 ${geometry.frame.outer.width + 60},${geometry.frame.outer.height / 2})`} fontSize={40 / zoom} className="font-semibold">
