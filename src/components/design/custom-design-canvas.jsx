...
import { CustomDesignToolbar } from "./custom-design-toolbar";
...
const DEFAULT_MATERIAL = 'aluminum';
const DEFAULT_GLASS = 'standard';
const DEFAULT_INFILL = 'glass';
export function CustomDesignCanvas({ fullscreenContainerRef }) {
    ...
    const [paintColor, setPaintColor] = useState(DEFAULT_STROKE_COLOR);
    const [paintTarget, setPaintTarget] = useState('frame');
    const [editingText, setEditingText] = useState(null);
    const [rotationIndicator, setRotationIndicator] = useState(null);
    const dragInfo = useRef(null);
    const panStart = useRef({ x: 0, y: 0 });
    const drawingElement = useRef(null);
    const lastDrawnElement = useRef(null);
    const canvasRef = useRef(null);
    // --- Undo/Redo ---
    const history = useRef([{ frames: [], mullions: [], dimensions: [], textBoxes: [] }]);
    ...
    useEffect(() => {
        ...
        frames.forEach(frame => {
            ...
            const frameProfile = 'standard'; // Assumption for now
            ...
            if (glassWidth > 0 && glassHeight > 0) {
                const glassArea = (glassWidth * glassHeight) / 1000000;
                if (infill === 'glass') {
        ...
        mullions.forEach(mullion => {
            ...
            const mullionProfile = 'standard'; // Assumption
        ...
        const consolidatedBom = bom.reduce((acc, item) => {
    ...
    const updateAndRecord = (updateAction) => {
        updateAction();
    ...
    const updateSelectedElementMaterial = (material) => {
        if (!selectedElement)
            return;
        updateAndRecord(() => {
    ...
    const updateSelectedElementInfillType = (infill) => {
    ...
    const updateSelectedElementGlass = (glass) => {
        if (!selectedElement || selectedElement.type !== 'frame')
            return;
        updateAndRecord(() => {
    ...
    const updateSelectedElementOpeningType = (opening) => {
        if (!selectedElement || selectedElement.type !== 'frame')
            return;
        updateAndRecord(() => {
            updateElements({
                frames: frames.map(f => f.id === selectedElement.id ? { ...f, opening, openState: 0 } : f)
            });
        });
    };
    ...
    const handleAddShape = (shapeType) => {
        ...
        if (shapeType === 'rectangle') {
            ...
            const offset = 20;
            const widthDim = { id: `dim_${Date.now()}_w`, type: 'dimension', x1: newFrame.x, y1: newFrame.y - offset, x2: newFrame.x + newFrame.width, y2: newFrame.y - offset, text: `${Math.round(newFrame.width)}mm`, parentId: newFrame.id };
            const heightDim = { id: `dim_${Date.now()}_h`, type: 'dimension', x1: newFrame.x - offset, y1: newFrame.y, x2: newFrame.x - offset, y2: newFrame.y + newFrame.height, text: `${Math.round(newFrame.height)}mm`, parentId: newFrame.id };
    ...
    const isPointOnLine = (line, point, threshold) => {
    ...
    const getLineHandleAtPoint = (point, line) => {
    ...
    const handleTextDoubleClick = (e, box) => {
        e.stopPropagation();
    ...
    const handleMouseMove = (e) => {
        ...
        if (mode === 'drawing' && drawingElement.current) {
            ...
            if (currentDrawingElement.type === 'frame') {
                ...
                if (Math.abs(targetPoint.x - snappedX) < snapThreshold)
                    targetPoint.x = snappedX;
                if (Math.abs(targetPoint.y - snappedY) < snapThreshold)
                    targetPoint.y = snappedY;
                const startX = drawingElement.current.x;
                const startY = drawingElement.current.y;
                ...
                const startX = drawingElement.current.x1;
                const startY = drawingElement.current.y1;
        ...
        if (mode === 'dragging' && dragInfo.current) {
            ...
            if (element.type === 'frame' || element.type === 'textbox') {
                ...
                if (element.type === 'frame') {
                    newElements.frames = frames.map(s => s.id === element.id ? { ...s, x: newX, y: newY } : s);
                }
                else {
                    ...
                    newElements.dimensions = dimensions.map(dim => {
                        ...
                        if (initialChild) {
                            return {
                                ...
                                rotationCenterX: element.x + element.width / 2,
                                rotationCenterY: element.y + element.height / 2
                            };
                        }
                        return dim;
                    });
                ...
                const newElements = { mullions: newMullions };
                if (dragInfo.current.childElements) {
                    newElements.dimensions = dimensions.map(dim => {
                        const initialChild = dragInfo.current.childElements.find(c => c.id === dim.id && c.type === 'dimension');
                        if (initialChild) {
                ...
                updateElements({ dimensions: dimensions.map(d => d.id === element.id ? { ...d, x1: newX1, y1: newY1, x2: newX2, y2: newY2 } : d) });
            }
        }
        if (mode === 'resizing' && dragInfo.current) {
            ...
            if (element.type === 'frame' || element.type === 'textbox') {
                ...
                if (element.type === 'frame') {
                    ...
                    updateElements({ textBoxes: textBoxes.map(s => s.id === element.id ? { ...s, x: newX, y: newY, width: Math.max(10, newWidth), height: Math.max(10, newHeight) } : s) });
                }
            }
            else if (element.type === 'mullion' || element.type === 'dimension') {
                const { elementStartX, elementStartY, elementEndX, elementEndY } = dragInfo.current;
                ...
                if (element.type === 'mullion') {
                    updateElements({ mullions: mullions.map(m => m.id === element.id ? { ...m, x1: finalX1, y1: finalY1, x2: finalX2, y2: finalY2 } : m) });
                }
                else { // dimension
                    const length = Math.hypot(finalX2 - finalX1, finalY2 - finalY1);
                    updateElements({ dimensions: dimensions.map(d => d.id === element.id ? { ...d, x1: finalX1, y1: finalY1, x2: finalX2, y2: finalY2, text: `${Math.round(length)}mm` } : d) });
                }
            }
        }
    ...
    const handleMouseUp = () => {
        ...
        if (mode === 'drawing' && drawingElement.current) {
            if (lastDrawnElement.current) {
                ...
                if (type === 'frame')
                    updateElements({ frames: frames.filter(f => f.id !== id) });
                if (type === 'mullion')
                    updateElements({ mullions: mullions.filter(m => m.id !== id) });
                if (type === 'dimension')
                    updateElements({ dimensions: dimensions.filter(d => d.id !== id) });
                if (type === 'textbox')
                    updateElements({ textBoxes: textBoxes.filter(tb => tb.id !== id) });
            }
    ...
    const renderedHandles = useMemo(() => {
        if (activeTool !== 'select' || !selectedElement)
            return null;
        if (selectedElement.type !== 'mullion' && selectedElement.type !== 'dimension')
            return null;
        const handleSize = HANDLE_SIZE / zoom;
        let handles = null;
        const element = [...mullions, ...dimensions].find(el => el.id === selectedElement.id);
        if (element)
            handles = getHandlesForLine(element);
        if (!handles)
            return null;
        return Object.entries(handles).map(([key, handle]) => (<rect key={key} x={handle.x} y={handle.y} width={handleSize} height={handleSize} fill={SELECTED_ELEMENT_COLOR} stroke="hsl(var(--card))" strokeWidth={1 / zoom} className="cursor-pointer" style={{ cursor: handle.cursor }} onMouseDown={(e) => {
    ...
    const getUnrotatedPoint = (point, element, center) => {
        if (!element.rotation)
            return point;
    ...
    return (<div className="relative w-full h-full bg-card rounded-lg border-2 border-dashed border-muted-foreground/30 shadow-sm flex flex-col items-center justify-center p-0 text-center overflow-hidden">
      <CustomDesignToolbar activeTool={activeTool} setActiveTool={setActiveTool} onClear={handleClear} onDeleteSelected={handleDeleteSelected} onZoomIn={handleZoomIn} onZoomOut={handleZoomOut} onUndo={handleUndo} onRedo={handleRedo} canUndo={undoRedoState.canUndo} canRedo={undoRedoState.canRedo} onIncreaseThickness={handleIncreaseThickness} onDecreaseThickness={handleDecreaseThickness} isElementSelected={!!selectedElement} onSetPaintColor={setPaintColor} paintTarget={paintTarget} onPaintTargetChange={setPaintTarget} onUpdateMaterial={updateSelectedElementMaterial} onUpdateInfillType={updateSelectedElementInfillType} onUpdateGlass={updateSelectedElementGlass} onUpdateOpeningType={updateSelectedElementOpeningType} selectedElementData={selectedElementData} onAddShape={handleAddShape} portalContainer={fullscreenContainerRef?.current}/>

      {rotationIndicator && (<div className="absolute pointer-events-none z-20 flex items-center justify-center bg-card border rounded-md p-1 px-2 text-xs font-mono shadow-lg" style={{
      ...
      <div ref={canvasRef} className={cn("w-full h-full", cursorClass)} style={{
        ...
        <svg width="100%" height="100%" className="relative z-0">
            ...
            if (el.color && el.color.startsWith('linear-gradient')) {
                ...
                if (colors.length >= 2) {
                    const gradientProps = { id: gradientId };
                    if (el.type === 'mullion') {
                    ...
                    return (<linearGradient key={gradientId} {...gradientProps}>
            ...
            return (<g key={shape.id} transform={`rotate(${shape.rotation || 0} ${center.x} ${center.y})`} onDoubleClick={() => toggleFrameOpenState(shape.id)}>
                    ...
                    if (isRotationHandle) {
                        return (<g key={key}>
                                        <line x1={center.x} y1={center.y + shape.height / 2} x2={handle.x} y2={handle.y} stroke={SELECTED_ELEMENT_COLOR} strokeWidth={1.5 / zoom}/>
                                        <circle cx={handle.x} cy={handle.y} r={handleSize} fill='white' stroke={SELECTED_ELEMENT_COLOR} strokeWidth={1.5 / zoom} className="cursor-pointer" style={{ cursor: handle.cursor }} onMouseDown={(e) => {
                                e.stopPropagation();
                                handleMouseDown(e);
                            }}/>
                                      </g>);
                    }
                    return (<rect key={key} x={handle.x} y={handle.y} width={handleSize} height={handleSize} fill={SELECTED_ELEMENT_COLOR} stroke={'white'} strokeWidth={1.5 / zoom} className="cursor-pointer" style={{ cursor: handle.cursor }} onMouseDown={(e) => {
                            e.stopPropagation();
                            handleMouseDown(e);
                        }}/>);
                })}
            ...
            return (<g key={mullion.id}>
                            <line x1={mullion.x1} y1={mullion.y1} x2={mullion.x2} y2={mullion.y2} stroke={stroke} strokeWidth={thickness} strokeLinecap="round"/>
                            {isSelected && (<line x1={mullion.x1} y1={mullion.y1} x2={mullion.x2} y2={mullion.y2} stroke={SELECTED_ELEMENT_COLOR} strokeWidth={SELECTION_OUTLINE_WIDTH / zoom} strokeLinecap="round" className="pointer-events-none"/>)}
                        </g>);
        })}
                {dimensions.map(dim => {
            ...
            return (<g key={dim.id} className="pointer-events-none" color={strokeColor} transform={`rotate(${dim.rotation || 0}, ${dim.rotationCenterX || 0}, ${dim.rotationCenterY || 0})`}>
                            <line x1={dim.x1} y1={dim.y1} x2={dim.x2} y2={dim.y2} stroke={strokeColor} strokeWidth={strokeWidth} markerStart="url(#dim-arrow)" markerEnd="url(#dim-arrow)" className="pointer-events-auto"/>
                            <text x={textX} y={textY} fill={strokeColor} fontSize={14 / zoom} textAnchor="middle" dominantBaseline="middle" transform={`rotate(${angle > 90 || angle < -90 ? angle + 180 : angle} ${textX} ${textY})`}>
                                {dim.text}
                            </text>
                        </g>);
        })}
            ...
            return (<g key={box.id} onDoubleClick={(e) => handleTextDoubleClick(e, box)} transform={`rotate(${rotation} ${center.x} ${center.y})`}>
                    ...
                    textAlign: 'center',
                            ...
                            {isSelected && (<rect x={box.x} y={box.y} width={box.width} height={box.height} fill="none" stroke={SELECTED_ELEMENT_COLOR} strokeWidth={SELECTION_OUTLINE_WIDTH / zoom} className="pointer-events-none"/>)}
                            {isSelected && Object.entries(getHandlesForRect(box)).map(([key, handle]) => {
                    const handleSize = HANDLE_SIZE / zoom;
                    const isRotationHandle = key === 'rotate';
                    if (isRotationHandle) {
                        return (<g key={key}>
                                        <line x1={center.x} y1={center.y + box.height / 2} x2={handle.x} y2={handle.y} stroke={SELECTED_ELEMENT_COLOR} strokeWidth={1.5 / zoom}/>
                                        <circle cx={handle.x} cy={handle.y} r={handleSize} fill='white' stroke={SELECTED_ELEMENT_COLOR} strokeWidth={1.5 / zoom} className="cursor-pointer" style={{ cursor: handle.cursor }} onMouseDown={(e) => {
                                e.stopPropagation();
                                handleMouseDown(e);
                            }}/>
       ...
       {editingText && (<textarea value={editingText.text} onChange={(e) => setEditingText(prev => prev ? { ...prev, text: e.target.value } : null)} onBlur={handleTextareaBlur} autoFocus onKeyDown={(e) => {
