...
const FramePiece = ({ position, args, color }) => (<Box position={position} args={args}>
    <meshStandardMaterial color={color}/>
  </Box>);
const Custom3DView = ({ frames = [], mullions = [], toggleCustomFrameOpenState }) => {
    ...
    return (<Canvas camera={{ position: [0, 0, 1.5], fov: 50 }} className="bg-background">
            ...
            const SashAndGlass = (<>
                            ...
                            {glassWidth > 0 && glassHeight > 0 && (<>
                                {infill === 'glass' && (<Box position={[0, 0, 0]} args={[glassWidth, glassHeight, 0.01]}>
                                        <meshStandardMaterial color={glassColor} transparent opacity={0.6} side={THREE.DoubleSide}/>
                                    </Box>)}
                                {infill === 'panel' && (<Box position={[0, 0, 0]} args={[glassWidth, glassHeight, 0.02]}>
                                        <meshStandardMaterial color={'#D3D3D3'} side={THREE.DoubleSide}/>
                                    </Box>)}
                                {infill === 'louver' && (<group>
                                    {Array.from({ length: Math.floor(glassHeight / (0.025)) }).map((_, i) => (<Box key={i} position={[0, (glassHeight / 2) - (i + 0.5) * 0.025, 0]} rotation={[-Math.PI / 6, 0, 0]} args={[glassWidth, 0.005, 0.025]}>
                                            <meshStandardMaterial color={'#A9A9A9'}/>
                                        </Box>))}
                                    </group>)}
                                {infill === 'brickwork' && (<Box position={[0, 0, 0]} args={[glassWidth, glassHeight, PROFILE_DEPTH * scale]}>
                                        <meshStandardMaterial color={'#8B4513'}/>
                                    </Box>)}
                                </>)}
                        </>);
            return (<group key={frame.id} rotation={[0, 0, -(frame.rotation || 0) * (Math.PI / 180)]} position={[centerX, centerY, 0]} onClick={(e) => {
                    if (isOperable) {
                        e.stopPropagation();
                        toggleCustomFrameOpenState?.(frame.id);
                    }
                }} onPointerOver={(e) => isOperable && e.nativeEvent.target && (e.nativeEvent.target.style.cursor = 'pointer')} onPointerOut={(e) => isOperable && e.nativeEvent.target && (e.nativeEvent.target.style.cursor = 'auto')}>
            ...
            return (<Box key={mullion.id} position={[center.x, center.y, center.z]} rotation={[0, 0, rotationZ]} args={[length, thickness, PROFILE_DEPTH * scale]}>
...
export function ThreeDVisualization({ geometry, parameters, customFrames, customMullions, panelOpenStates, panelOffsets, togglePanelOpenState, updatePanelOffsets, toggleCustomFrameOpenState }) {
    ...
    const handlePointerMove = (event) => {
        if (!draggedPanel || !isSliding || !geometry || !updatePanelOffsets || !event.point)
            return;
        event.stopPropagation();
    ...
    return (<Canvas camera={{ position: [0, 0, frameWidth * 1.5], fov: 50 }} className="bg-background" onPointerMove={handlePointerMove} onPointerUp={handlePointerUp} onPointerLeave={handlePointerUp}>
      <ambientLight intensity={Math.PI / 2}/>
      <spotLight position={[10, 10, 10]} angle={0.15} penumbra={1} decay={0} intensity={Math.PI}/>
      <pointLight position={[-10, -10, -10]} decay={0} intensity={Math.PI}/>
      ...
      <group>
        {/* Main Frame */}
        <FramePiece position={[0, (frameHeight - frameThickness) / 2, 0]} args={[frameWidth, frameThickness, PROFILE_DEPTH * scale]} color={frameColor}/>
        <FramePiece position={[0, (-frameHeight + frameThickness) / 2, 0]} args={[frameWidth, frameThickness, PROFILE_DEPTH * scale]} color={frameColor}/>
        <FramePiece position={[(-frameWidth + frameThickness) / 2, 0, 0]} args={[frameThickness, frameHeight - 2 * frameThickness, PROFILE_DEPTH * scale]} color={frameColor}/>
        <FramePiece position={[(frameWidth - frameThickness) / 2, 0, 0]} args={[frameThickness, frameHeight - 2 * frameThickness, PROFILE_DEPTH * scale]} color={frameColor}/>
        ...
        {panels.map((panel, index) => {
            ...
            let panelGroupPosition = [panelX, panelY, baseZOffset];
            if (isSliding) {
            ...
            return (<group key={index} position={panelGroupPosition} onClick={handlePanelClick(index)} onPointerDown={handlePointerDown(index)}>
              <group position={rotationPivot} rotation={[rotationAxis === 'x' ? rotationAngle : 0, rotationAxis === 'y' ? rotationAngle : 0, 0]}>
                <group position={[-rotationPivot[0], -rotationPivot[1], -rotationPivot[2]]}>
                  {/* Panel Sash */}
