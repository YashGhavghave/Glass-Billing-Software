...
export const DesignManager = ({ designs, activeDesign, addDesign, removeDesign, selectDesign, updateDesignName }) => {
    ...
    return (<Card className="bg-sidebar-accent border-sidebar-border">
      ...
      <CardContent>
        <div className="flex flex-col gap-2">
            {designs.length === 0 ? (<p className="text-sm text-sidebar-accent-foreground/70 text-center py-2">No windows in this project.</p>) : designs.map(design => (<div key={design.id} onClick={() => editingId !== design.id && selectDesign(design.id)} className={cn("flex items-center justify-between p-2 rounded-md cursor-pointer transition-colors", activeDesign?.id === design.id ? "bg-sidebar-primary" : "hover:bg-sidebar-accent-foreground/5")}>
                    {editingId === design.id ? (<Input value={editingName} onChange={e => setEditingName(e.target.value)} className="h-7 text-sm bg-card" autoFocus onKeyDown={e => {
                    if (e.key === 'Enter')
                        handleSave(design.id);
                    if (e.key === 'Escape')
                        handleCancel();
                }} onBlur={() => handleSave(design.id)} suppressHydrationWarning/>) : (<span className={cn("text-sm font-medium", activeDesign?.id === design.id ? "text-sidebar-primary-foreground" : "text-sidebar-accent-foreground")}>{design.name}</span>)}
