"use client";
import { MousePointer, PenSquare, Spline, Combine, Ruler, Undo, Redo, ZoomIn, ZoomOut, Move, Trash2, Plus, Minus, Frame as FrameIcon, GlassWater, LayoutGrid, Shapes, Palette, Droplet, Type, BookOpenCheck, } from "lucide-react";
...
export function CustomDesignToolbar({ activeTool, setActiveTool, onClear, onDeleteSelected, onZoomIn, onZoomOut, onUndo, onRedo, canUndo, canRedo, onIncreaseThickness, onDecreaseThickness, isElementSelected, onSetPaintColor, paintTarget, onPaintTargetChange, onUpdateMaterial, onUpdateInfillType, onUpdateGlass, onUpdateOpeningType, selectedElementData, onAddShape, portalContainer, }) {
      ...
      <div className="absolute top-4 left-2 right-2 sm:w-auto sm:left-1/2 sm:-translate-x-1/2 z-10">
        <div className="flex items-center justify-center flex-wrap gap-1 p-1.5 bg-card border rounded-lg shadow-lg">
            {tools.map(tool => (<Tooltip key={tool.id}>
                    ...
                    <Button variant="ghost" size="icon" className={cn("h-9 w-9", activeTool === tool.id && "bg-primary/10 text-primary")} onClick={() => setActiveTool(tool.id)} disabled={disabledTools.includes(tool.id)}>
                     ...
                     <div className="space-y-4">
                        <div>
                            ...
                            <div className="grid grid-cols-6 gap-2">
                                {materialColors.map(color => (<Button key={color} variant="outline" size="icon" className="h-7 w-7 rounded-md border-muted-foreground/20 p-0" onClick={() => onSetPaintColor(color)}>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                {materialPresets.map((material, index) => (<Tooltip key={index}>
                                    <TooltipTrigger asChild>
                                      <Button variant="outline" className="h-7 w-full rounded-md p-0 border-muted-foreground/20" style={{ background: material.value }} onClick={() => onSetPaintColor(material.value)}/>
                                    </TooltipTrigger>
                        ...
                        <div>
                            ...
                            <div className="flex items-center gap-2">
                                <Input type="color" className="h-8 w-8 p-1 cursor-pointer" onChange={(e) => onSetPaintColor(e.target.value)} defaultValue="#343434"/>
                                <Input type="text" placeholder="#RRGGBB" className="h-8" onChange={(e) => {
            ...
            <Tooltip>
                <TooltipTrigger asChild>
                    <div>
                        <Select value={paintTarget} onValueChange={onPaintTargetChange}>
            ...
            <Separator orientation="vertical" className="h-6 mx-1"/>
             ...
             {thicknessActions.map(tool => (<Tooltip key={tool.id}>
                    <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" className="h-9 w-9" onClick={tool.id === 'thicker' ? onIncreaseThickness : onDecreaseThickness} disabled={!isElementSelected}>
                        <tool.icon className="h-5 w-5"/>
                    </Button>
                    </TooltipTrigger>
                    <TooltipContent side="bottom">
            ...
            <div className="hidden sm:flex items-center gap-1">
                <Tooltip>
                    <TooltipTrigger asChild>
                        <div>
                            <Select value={selectedElementData && "material" in selectedElementData ? selectedElementData.material : undefined} onValueChange={onUpdateMaterial} disabled={!selectedElementData || !('material' in selectedElementData)}>
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        <div>
                            <Select value={selectedElementData && "infill" in selectedElementData ? selectedElementData.infill : undefined} onValueChange={onUpdateInfillType} disabled={!selectedElementData || !('infill' in selectedElementData)}>
                                ...
                                <SelectContent container={portalContainer}>
                                    {infillTypes.map(i => (<SelectItem key={i.value} value={i.value}>{i.label}</SelectItem>))}
                                </SelectContent>
                            </Select>
                        </div>
                    </TooltipTrigger>
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        <div>
                            <Select value={selectedElementData && "glass" in selectedElementData ? selectedElementData.glass : undefined} onValueChange={onUpdateGlass} disabled={!selectedElementData || !('glass' in selectedElementData) || (selectedElementData && 'infill' in selectedElementData && selectedElementData.infill !== 'glass')}>
                                ...
                                <SelectContent container={portalContainer}>
                                    {glassTypes.map(g => (<SelectItem key={g.value} value={g.value}>{g.label}</SelectItem>))}
                                </SelectContent>
                            </Select>
                        </div>
                    </TooltipTrigger>
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        <div>
                            <Select value={selectedElementData && "opening" in selectedElementData ? selectedElementData.opening : undefined} onValueChange={onUpdateOpeningType} disabled={!selectedElementData || !('opening' in selectedElementData)}>
                                ...
                                <SelectContent container={portalContainer}>
                                    {openingTypes.map(o => (<SelectItem key={o.value} value={o.value}>{o.label}</SelectItem>))}
                                </SelectContent>
                            </Select>
                        </div>
                    </TooltipTrigger>
            ...
            {navigation.map(tool => (<Tooltip key={tool.id}>
                    <TooltipTrigger asChild>
                    <Button variant="ghost" size="icon" className={cn("h-9 w-9", activeTool === tool.id && "bg-primary/10 text-primary")} onClick={() => {
                if (tool.id === 'pan') {
            ...
            return (<Tooltip key={action.id}>
                        <TooltipTrigger asChild>
                        <Button variant="ghost" size="icon" className={cn("h-9 w-9", action.id === 'clear' && 'text-destructive/80 hover:text-destructive hover:bg-destructive/10')} onClick={actionFunc} disabled={isDisabled}>
