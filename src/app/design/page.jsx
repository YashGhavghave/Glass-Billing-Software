...
export default function DesignPage() {
    ...
    const handleFullscreen = useCallback((view) => {
      ...
      <div className={cn("flex h-screen w-full bg-background", fullscreenView && 'bg-card')}>
        <div className={cn(fullscreenView && 'hidden')}>
          <Sidebar className="flex flex-col">
            <SidebarContent className="flex-1">
              <DesignControlPanel designs={designs} activeDesign={activeDesign} addDesign={addDesign} removeDesign={removeDesign} selectDesign={selectDesign} updateDesignName={updateDesignName} updateParameters={updateParameters}/>
            </SidebarContent>
          </Sidebar>
        </div>
        <SidebarInset className={cn("flex flex-col", fullscreenView && 'p-0 m-0 rounded-none min-h-screen')}>
          ...
          <main className={cn("flex-1 overflow-auto p-4 md:p-6", fullscreenView && 'p-0')}>
            {activeDesign ? (<div className={cn("grid h-full grid-rows-[1fr_auto] gap-4", fullscreenView && 'gap-0')}>
                <div className={cn("grid grid-cols-1 gap-4 lg:grid-cols-2", fullscreenView && 'grid-cols-1 grid-rows-1 h-full gap-0')}>
                  <Card ref={view2dRef} className={cn("flex flex-col", (fullscreenView && fullscreenView !== '2d') && 'hidden', fullscreenView === '2d' && 'border-0 rounded-none shadow-none')}>
                    ...
                    <CardContent className="flex-1 overflow-hidden p-0">
                      ...
                      <div className={cn('h-full w-full', isCustomMode && 'hidden')}>
                        <Visualization key={activeDesign.id} geometry={activeDesign.geometry} isProcessing={isProcessing} parameters={activeDesign.parameters} panelOpenStates={activeDesign.panelOpenStates || []} panelOffsets={activeDesign.panelOffsets || []} togglePanelOpenState={togglePanelOpenState} updatePanelOffsets={updatePanelOffsets}/>
                      </div>
                    </CardContent>
                  </Card>
                  <Card ref={view3dRef} className={cn("flex flex-col", (fullscreenView && fullscreenView !== '3d') && 'hidden', fullscreenView === '3d' && 'border-0 rounded-none shadow-none')}>
                    ...
                    <CardContent className="flex-1 overflow-hidden p-0">
                      <ThreeDVisualization geometry={activeDesign.geometry} parameters={activeDesign.parameters} customFrames={isCustomMode ? activeDesign.frames : undefined} customMullions={isCustomMode ? activeDesign.mullions : undefined} panelOpenStates={activeDesign.panelOpenStates} panelOffsets={activeDesign.panelOffsets} togglePanelOpenState={togglePanelOpenState} updatePanelOffsets={updatePanelOffsets} toggleCustomFrameOpenState={toggleCustomFrameOpenState}/>
                    </CardContent>
                  </Card>
                </div>
                <div className={cn(fullscreenView && 'hidden')}>
