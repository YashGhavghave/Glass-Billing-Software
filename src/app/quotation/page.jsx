...
export default function QuotationPage() {
    ...
    const [state, dispatch] = useFormState(createQuoteWithDesigns, undefined);
    useEffect(() => {
        if (state?.error) {
            toast({
                variant: 'destructive',
    ...
    if (designs.length === 0) {
        return (<div className="flex items-center justify-center min-h-screen bg-muted/40 p-4">
            ...
            <CardContent>
                <form action={dispatch} className="grid gap-4">
                    <div className="grid gap-2">
                        ...
                        <Input id="partyName" name="name" placeholder="Enter customer's name" required/>
                    </div>
                    <div className="grid gap-2">
                        <Label htmlFor="phone">Phone Number</Label>
                        <Input id="phone" name="phone" placeholder="Enter customer's phone number" required/>
                    </div>
                    <SubmitButton />
